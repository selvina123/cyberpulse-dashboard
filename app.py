...
with st.sidebar:
    ...
    <div style="margin-top:10px;">
        <p class="menu-item">🔍 Discovery</p>
        <p class="menu-item menu-active">🛡️ API Security</p>
        <p class="menu-item">☁️ Cloud Infrastructure</p>
        <p class="menu-item">📦 Inventory</p>
        <p class="menu-item">⚔️ Attack Paths</p>
        <p class="menu-item">🧩 Vulnerabilities</p>
        <p class="menu-item">✅ Compliance</p>
    </div>
    """, unsafe_allow_html=True)
...
# Normalize timestamps and prepare features
df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
...
with colB:
    ...
    fig_map.update_traces(
        marker=dict(opacity=0.8, line=dict(width=2, color="rgba(255,255,255,0.4)"))
    )
...
# =================== ROW 2: Trends + Top IPs ===================
# =================== ROW 2: Trends + Event Type Split ===================
colC, colD = st.columns((2,1))

with colC:
    # Events per Minute (line chart with neon purple)
...
with colD:
    ...
    fig_pie = px.pie(
        ...
        color="event_type",
        color_discrete_map={
            "failed_login": "#636EFA",       # Blue
            "successful_login": "#EF553B",   # Red
            "port_scan": "#00CC96",          # Green
            "suspicious_login": "#AB63FA"    # Purple
        }
    )
...
# =================== ROW 3: Table ===================
st.markdown('<div class="panel-title">Threat Details</div>', unsafe_allow_html=True)

if alerts.empty:
    st.info("✅ No detections right now.")
else:
    show = alerts.copy()
    show["time"] = pd.to_datetime(show["time"]).dt.strftime("%Y-%m-%d %H:%M:%S")

    # Remove repeating values (replace duplicates with "")
    show = show.reset_index(drop=True)
    for col in ["src_ip", "rule", "evidence", "severity"]:
        show[col] = show[col].mask(show[col].eq(show[col].shift()))
