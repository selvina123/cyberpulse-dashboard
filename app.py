...
st.set_page_config(page_title="CyberPluse Dashboard", page_icon="🛡️", layout="wide")
...
def detect_alerts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not failed.empty:
        ...
        for _,r in bf.iterrows():
            alerts.append({"time":r["timestamp"],"src_ip":r["src_ip"],"rule":"Brute Force (>=6/2min)","evidence":f"count={int(r['count'])}","severity":"medium"})
    ...
    for _,r in ps.iterrows():
        alerts.append({"time":r["timestamp"],"src_ip":r["src_ip"],"rule":"Port Scan (>=12 ports/2min)","evidence":f"unique_ports={int(r['unique_ports'])}","severity":"medium"})

    sus = work[work["event_type"]=="suspicious_login"]
    for _,r in sus.iterrows():
        alerts.append({"time":r["timestamp"],"src_ip":r.get("src_ip",""),"rule":"Suspicious Login","evidence":f"user={r.get('username','')}","severity":"high"})
...
if menu == "📊 Dashboard":
    ...
    with colC:
        ...
        fig_line.update_traces(line=dict(color="#C77DFF", width=2))  # light purple line
    ...
    st.header("🔍 Discovery")
    st.write("Discovery and reconnaissance events")
    st.dataframe(df[df["event_type"].isin(["port_scan","suspicious_login"])].head(20))

elif menu == "🛡️ API Security":
    st.header("🛡️ API Security")
    st.write("API login activity and anomalies")
    st.dataframe(df[df["event_type"].isin(["failed_login","successful_login","suspicious_login"])].head(20))

elif menu == "☁️ Cloud Infrastructure":
    st.header("☁️ Cloud Infrastructure")
    st.write("Cloud infrastructure detections (DDoS, login attempts)")
    st.dataframe(df[df["event_type"].isin(["port_scan","failed_login"])].head(20))

elif menu == "📦 Inventory":
    st.header("📦 Inventory")
    st.write("Active IPs and users")
    st.dataframe(df[["src_ip","username","event_type","severity"]].head(20))

elif menu == "⚔️ Attack Paths":
    st.header("⚔️ Attack Paths")
    st.write("Suspicious logins and insider threats")
    st.dataframe(df[df["event_type"].isin(["suspicious_login"])].head(20))

elif menu == "🧩 Vulnerabilities":
    st.header("🧩 Vulnerabilities")
    st.write("Potential malware or phishing detections (simulated)")
    st.dataframe(df.head(20))

elif menu == "✅ Compliance":
    st.header("✅ Compliance")
    st.write("Compliance metrics overview")
    comp = df["severity"].value_counts().reset_index()
    comp.columns = ["Severity","Count"]
    st.bar_chart(comp.set_index("Severity"))
